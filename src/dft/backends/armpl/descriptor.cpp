...
void descriptor<prec, dom>::commit(backend_selector<backend::armpl> selector) {
    if (!pimpl_ || pimpl_->get_queue() != selector.get_queue()) {
        if (pimpl_) {
            pimpl_->get_queue().wait();
        }
        pimpl_.reset(armpl::create_commit(*this, selector.get_queue()));
    }
    pimpl_->commit(values_);
}

template void descriptor<precision::SINGLE, domain::COMPLEX>::commit(
    backend_selector<backend::armpl>);
template void descriptor<precision::SINGLE, domain::REAL>::commit(backend_selector<backend::armpl>);
template void descriptor<precision::DOUBLE, domain::COMPLEX>::commit(
    backend_selector<backend::armpl>);
template void descriptor<precision::DOUBLE, domain::REAL>::commit(backend_selector<backend::armpl>);

...
// BUFFER version
// Forward a ARMPL DFT call to the backend, checking that the commit impl is valid.
template <dft::precision prec, dft::domain dom>
...
template <dft::precision prec, dft::domain dom>
auto get_fwd_plan(dft::detail::commit_impl<prec, dom>* commit) {
    using planType = typename std::conditional_t<prec == oneapi::math::dft::precision::SINGLE,
                                                 fftwf_plan, fftw_plan>;
    return static_cast<std::optional<planType>*>(commit->get_handle())[0].value();
}
} // namespace detail

template <typename in_data_type, typename out_data_type, typename planType>
void ArmplExecuteForward(const planType plan, in_data_type* in, out_data_type* out) {
...
template <typename in_data_type, typename out_data_type, typename planType>
void ArmplExecuteForwardSplit(const planType plan, in_data_type* r_in, in_data_type* i_in,
    ...
    if constexpr (is_single) {
        ...
        fftwf_execute_split_dft(plan, r_in, i_in, r_out, i_out);
        std::cout << "for i_out ";
        for (int i = 0; i < 8; i++)
            std::cout << i_out[i] << " ";
        std::cout << std::endl;
        ...
        fftw_execute_split_dft(plan, r_in, i_in, r_out, i_out);
    }
}
...
ONEMATH_EXPORT void compute_forward(descriptor_type& desc,
    ...
    auto plan = detail::get_fwd_plan(commit_handle);
    auto offsets = detail::get_offsets_fwd(commit_handle);
    ...
    cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_kernel_inplace>(cgh, [=]() {
            ArmplExecuteForward(plan, detail::acc_to_ptr(inout_acc) + offsets[0],
...
ONEMATH_EXPORT void compute_forward(descriptor_type& desc,
    ...
    cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_kernel_split_inplace>(cgh, [=]() {
            ArmplExecuteForwardSplit(plan, detail::acc_to_ptr(re_acc) + offsets[0],
...
ONEMATH_EXPORT void compute_forward(descriptor_type& desc,
    ...
    cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_kernel_outofplace>(cgh, [=]() {
            ...
            ArmplExecuteForward(plan, in_ptr + offsets[0],
...
ONEMATH_EXPORT void compute_forward(descriptor_type& desc,
    ...
    cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_kernel_split_outofplace>(cgh, [=]() {
            ...
            ArmplExecuteForwardSplit(plan, inre_ptr + offsets[0], inim_ptr + offsets[0],
...
ONEMATH_EXPORT sycl::event compute_forward(descriptor_type& desc, fwd<descriptor_type>* inout,
    ...
    return cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_usm_kernel_inplace>(
            cgh, [=]() { ArmplExecuteForward(plan, inout + offsets[0], inout + offsets[1]); });
    });
}
...
ONEMATH_EXPORT sycl::event compute_forward(descriptor_type& desc, scalar<descriptor_type>* inout_re,
    ...
    return cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_usm_kernel_split_inplace>(cgh, [=]() {
            ArmplExecuteForwardSplit(plan, inout_re + offsets[0], inout_im + offsets[0],
...
ONEMATH_EXPORT sycl::event compute_forward(descriptor_type& desc, fwd<descriptor_type>* in,
    ...
    return cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_usm_kernel_outofplace>(
            cgh, [=]() { ArmplExecuteForward(plan, in + offsets[0], out + offsets[1]); });
    });
}
...
ONEMATH_EXPORT sycl::event compute_forward(descriptor_type& desc, scalar<descriptor_type>* in_re,
    ...
    return cpu_queue.submit([&](sycl::handler& cgh) {
        ...
        detail::host_task<class host_usm_kernel_split_outofplace>(cgh, [=]() {
            ArmplExecuteForwardSplit(plan, in_re + offsets[0], in_im + offsets[0],
